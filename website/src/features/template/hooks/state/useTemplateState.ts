 ...
 * snapshot (`TemplateSnapshotStateProvider`, submission fill-out). Every
...
export function useTemplateState(): TemplateState {
  ...
  if (!state) {
    throw new Error(
      "useTemplateState must be used inside a template-state provider (TemplateProvider or TemplateSnapshotStateProvider)",
    );
  }
  return state;
}
