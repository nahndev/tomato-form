...
- [x] Update `WIDGET_VALUE_PROPERTY_REGISTRY` using new constants `ValueType`
...
- `website/src/features/board/constants/column/valueProperties.ts` → `valueTypes.ts` (renamed, matching `server/src/widget-value/widget-value.types.ts`'s `ValueType`/`VALUE_TYPE` naming): `ValueProperty`/`ValueProperty` const → `ValueType`; `WIDGET_VALUE_PROPERTY_REGISTRY` → `WIDGET_VALUE_TYPE_REGISTRY`; `getWidgetValueProperties` → `getWidgetValueTypes`; `getPropertyDisplayTypes` → `getValueTypeDisplayTypes`; `VALUE_PROPERTY_LABELS` → `VALUE_TYPE_LABELS`. `getWidgetDisplayTypes` unchanged. Every consumer updated: `TemplateWidgetSelect.tsx`, `ChartSettingPopup.tsx`, `useTemplateWidgetsByDisplayTypes.ts`, `column.ts`, `displayTypeHelper.ts`, `boardColumnWidgets.ts`, `itemKey.ts`, plus their tests. The `BoardColumnItem`/`SelectedColumnItem`/`DisplayTypeEntry` field itself stays named `property` (it's the item's own field name, not the type name) and is now typed as `ValueType`. `docs/WIDGET.md`'s file map and `widget-value.types.ts`'s doc comment updated to point at the new path/name.
- Out of scope for this item: the board module's own `ValueProperty`/`ValuePropertyDto` (`server/src/board/board.types.ts`, `server/src/board/dto/board-column.dto.ts`) are a separate, still-`ValueProperty`-named pair from a different backend module than `widget-value`'s `ValueType` - left as-is since renaming them wasn't asked for and isn't covered by this checklist item; flagging as a possible follow-up if the two are meant to converge.
